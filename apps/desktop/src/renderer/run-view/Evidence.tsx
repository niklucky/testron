import { Button, Icon, SectionLabel, type IconName } from '../design';
      ...
      <li key={artifact.id}>
        ...
        >
          ...
          <span className="ui-mono shrink-0 text-xs text-ink-3">
            {artifact.captured ? artifact.size : 'not captured'}
          </span>
        </button>
      </li>
    ))}
...
const FailureShot = ({ label }: { label: string }) => (
  <div className="overflow-hidden rounded-lg border border-line">
    <svg viewBox="0 0 320 190" className="w-full" role="img" aria-label="Failure screenshot">
...
export const Evidence = ({
  ...
  <aside className="ui-scroll min-h-0 space-y-4 overflow-y-auto border-l border-line px-3 py-3">
    <section>
      ...
      <Artifacts artifacts={run.artifacts} onOpen={(name) => onLog(`Opened ${name} · ${run.id}`)} />
    </section>

    {failingLabel && (
      <section>
        <SectionLabel className="mb-1.5 block">Failure screenshot</SectionLabel>
        <FailureShot label={failingLabel} />
        <p className="mt-1.5 text-xs text-ink-3">Full page, taken the moment the step failed.</p>
      </section>
    )}
    ...
    <section>
      <div className="mb-1.5 flex items-center gap-2">
        ...
        <span className="ml-auto text-xs text-ink-3">
          {run.console.filter((line) => line.level === 'error').length} errors
        </span>
      </div>
      <Console lines={run.console} />
      <p className="mt-1.5 text-xs text-ink-3">
        Shown from the page under test — the runner does not persist this yet.
      </p>
    </section>

    <section>
      <SectionLabel className="mb-1.5 block">Environment</SectionLabel>
      <Facts run={run} />
    </section>
  </aside>
);
